...
DOM 结构和选择器详见 [DOM_REFERENCE.md](DOM_REFERENCE.md)。

操作步骤:

1. 使用 `navigate_page` 导航到主 URL（`{type: "url", url: "主URL"}`）
2. 等待 `#sitemapTreeContainer` 加载
3. 使用 `evaluate_script` 提取所有页面信息（脚本见 [DOM_REFERENCE.md](DOM_REFERENCE.md)）
4. 返回页面列表数组
...
详见 [OUTPUT_TEMPLATE.md](OUTPUT_TEMPLATE.md)。
